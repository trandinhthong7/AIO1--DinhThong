...
if ! docker inspect "$CONTAINER" > /dev/null 2>&1; then
  ...
  # Clear deterministic lab fault markers before restarting. Docker restart
  # preserves the writable container layer, including files under /tmp.
  docker exec "$CONTAINER" rm -f /tmp/chaos_latency_ms 2>/dev/null || true

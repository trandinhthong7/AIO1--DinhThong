...
def detect_drift(
    ...
    if not per_feature:
        for metric in result_dict.get("metrics", [])[1:]:
            candidate = metric.get("result", {}).get("drift_by_columns", {})
            if candidate:
                per_feature = candidate
                break
    drifted_features = [
...
def log_to_mlflow(result: DriftResult, experiment_name: str = "anomaly-detection-drift") -> None:
    ...
    tracking_uri = os.environ.get("MLFLOW_TRACKING_URI", local_store.as_uri())
...
def main():
    ...
    parser.add_argument(
        ...
        default=True,
        help="Log drift score to MLflow (use --no-log-mlflow to disable)",
    )
